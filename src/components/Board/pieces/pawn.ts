...
export const pawn = (identity: -1 | 1): Piece => ({
  ...
  move: (location, board) => {
    ...
    if (board[coord2idx(leftDiag)].piece && board[coord2idx(leftDiag)].piece?.identity !== identity) {
      ret.push(leftDiag);
    }

    const rightDiag = {x: location.x + 1, y: location.y - identity * 1};
    if (board[coord2idx(rightDiag)].piece && board[coord2idx(rightDiag)].piece?.identity !== identity) {
      ret.push(rightDiag);
    }
